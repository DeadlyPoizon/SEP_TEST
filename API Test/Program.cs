...
namespace ConsoleTests
{
    public class Program
    {
        ...
        {
            string QUERY_URL =
                "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=IBM&apikey=BHS2KBKO7OUQXJFK";

            HttpWebRequest req = (HttpWebRequest)WebRequest.Create(QUERY_URL);
            HttpWebResponse resp = (HttpWebResponse)req.GetResponse();

            StreamReader sr = new StreamReader(resp.GetResponseStream());
            string results = sr.ReadToEnd();
            sr.Close();
            File.WriteAllText("data.csv", results);
...
namespace StockProject
{
    class Program
    {
        ...
        {
            AVConnection conn = new AVConnection("BHS2KBKO7OUQXJFK");
